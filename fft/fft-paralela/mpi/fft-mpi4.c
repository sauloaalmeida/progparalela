...
void calculoButterflyBloco(float dataBloco[], unsigned long m,unsigned long mmax, unsigned long istep, double wr, double wi, unsigned long tamBloco){
    ...
    //printf("          >>> calculaButterfly 2 -> wi:%f wr:%f istep:%lu mmax:%lu\n",wi,wr,istep, mmax);
    
    for (i=m;bloco<=tamBloco;i+=istep) {
        j=i+mmax;                                
        //printf("          >>> calculaButterfly 2 -> wi:%f wr:%f istep:%lu mmax:%lu i:%lu j:%lu\n",wi,wr,istep, mmax, i, j);
...
void calculaButterflyMPI(DataTransporte *dataTransporte, double wr, double wi, unsigned long tamBloco){
    ...
    //printf("          >>> CalculaButterflyMPI 2 -> wi:%f wr:%f\n",wi,wr);
    
    for (i=0;bloco<=tamBloco;i+=4) {
        
        //printf("          >>> CalculaButterflyMPI 2 -> wi:%f wr:%f i:%lu\n",wi,wr, i);
                ...
                if (qtdProcessos > 1 && qtdElementos/mmax >= totalPeso) {
                    ...
                    //printf("     tamBloco:%lu\n",tamBloco);
                       ...
                       free(dadosExecucaoProcesso);
                        ...
                        //printf("     processo %d - alocou %lu slotes de memoria tipo DataTransporte para os dados que serao transportados\n",rank, tamBloco * 4 );
                        ...
                        for (i=m;i<=tamArray;i+=istep) {
                            ...
                            //printf("\t### FOR2 -> bloco:%lu, count:%lu, i:%lu, qtdElementos:%lu, istep:%lu\n",bloco,count,i,qtdElementos,istep);
                            ...
                            if(bloco==tamBloco){
                                //envia os dados
                                //printf("#########################  Imprimindo pacote antes de enviar: bloco:%lu count:%lu \n",bloco,count);
                                //imprimeVetorPackage(dataTransporte,count);
                                
                                //manda para cada processo o seu bloco de processamento
                                MPI_Send(dataTransporte, count, dataTransporteType, blocoAtual, 123, MPI_COMM_WORLD);
                    ...
                    for (blocoAtual=0; blocoAtual<qtdProcessos; blocoAtual++) {
                        //printf("     recebendo resultado trabalho do bloco:%lu\n",blocoAtual);
                        ...
                        MPI_Recv(dataTransporte, tamBloco*4, dataTransporteType, blocoAtual, 123, MPI_COMM_WORLD,&status);
                        
                        //atualiza o vetor principal
                        unsigned long count2;
                        //printf("     Atualizando vetor principal\n");
        ...
        //printf("Dados de execucao inicializados:  quantidade de elementos: %lu, tamArray: %lu, pesoThreads %d, totalPeso %lu, no processo: %d\n",qtdElementos,tamArray,pesoThreads,totalPeso,rank);
          while(1){
             ...
             //imprimeVetorPackage(dataTransporte,packageSize);
              ...
              //processam o fft 
              calculaButterflyMPI(dataTransporte,dadosExecucaoProcesso[0].wr,dadosExecucaoProcesso[0].wi,packageSize); 
              
              //imprime os dados apos buterfly
              //printf("#########################  Imprimindo pacote recebido apos butterfly\n");
              //imprimeVetorPackage(dataTransporte,packageSize);
             
              //devolvem para o raiz o seu bloco de processamento
              MPI_Send(dataTransporte, packageSize, dataTransporteType, root, 123, MPI_COMM_WORLD);
